...
interface TurboLakeAnimationProps {
  target: number;
  duration: number;
  sectionIndex: number;
...
export function TurboLakeAnimation({ 
  target, 
  duration, 
  sectionIndex, 
  currentSection 
}: TurboLakeAnimationProps) {
  ...
  useEffect(() => {
    fetch('/data/lakes/lakes_1990.geojson')
      ...
      .then(data => {
        if (data.features) {
          // Use ALL lakes, duplicate smaller ones to increase density
          const allLakes = data.features.filter((f: any) => f.geometry && f.geometry.type === 'Polygon');
          
          // Duplicate smaller lakes to increase visual density
          const duplicatedLakes = [];
          allLakes.forEach((lake: any, idx: number) => {
            const area = lake.properties?.area_km2 || 0.1;
            // Add original
            duplicatedLakes.push({ ...lake, uniqueId: `${idx}-0` });
            
            // Add more copies for smaller lakes
            if (area < 0.5) {
              for (let i = 1; i < 4; i++) {
                duplicatedLakes.push({ ...lake, uniqueId: `${idx}-${i}` });
              }
            } else if (area < 2) {
              for (let i = 1; i < 2; i++) {
                duplicatedLakes.push({ ...lake, uniqueId: `${idx}-${i}` });
          ...
          const positions = sortedLakes.map((lake: any, index: number) => {
            ...
            return {
              ...
              appearDelay: seededRandom(seed + 4) * 800 // Random delay up to 0.8 seconds (2.5x faster)
            };
          });
  ...
  const animateCount = () => {
    ...
    const increment = maxTarget / ((duration / 2.5) / 16); // 2.5x faster
  ...
  return (
    <>
      {/* Lake outlines in background - super populated */}
