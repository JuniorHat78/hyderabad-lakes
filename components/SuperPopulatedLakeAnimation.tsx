...
interface SuperPopulatedLakeAnimationProps {
  target: number;
  duration: number;
  sectionIndex: number;
...
export function SuperPopulatedLakeAnimation({ 
  target, 
  duration, 
  sectionIndex, 
  currentSection 
}: SuperPopulatedLakeAnimationProps) {
        ...
        if (data.features) {
          ...
          const positions = sortedLakes.map((lake: any, index: number) => {
            ...
            return {
              ...
              appearDelay: seededRandom(seed + 4) * 2000 // Random delay up to 2 seconds
            };
          });
