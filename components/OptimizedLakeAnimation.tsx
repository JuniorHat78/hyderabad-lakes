...
interface OptimizedLakeAnimationProps {
  target: number;
  duration: number;
  sectionIndex: number;
...
export function OptimizedLakeAnimation({ 
  target, 
  duration, 
  sectionIndex, 
  currentSection 
}: OptimizedLakeAnimationProps) {
  ...
  useEffect(() => {
    fetch('/data/lakes/lakes_1990.geojson')
      ...
      .then(data => {
        if (data.features) {
          // Use FEWER lakes for better performance, but still visually dense
          const allLakes = data.features.filter((f: any) => f.geometry && f.geometry.type === 'Polygon');
          
          // Take only the first 300 lakes and duplicate some for visual density
          const selectedLakes = allLakes.slice(0, 300);
          const duplicatedLakes = [];
          
          selectedLakes.forEach((lake: any, idx: number) => {
            const area = lake.properties?.area_km2 || 0.1;
            // Add original
            duplicatedLakes.push({ ...lake, uniqueId: `${idx}-0` });
            
            // Add fewer copies for smaller lakes to maintain performance
            if (area < 0.5 && idx < 100) {
              duplicatedLakes.push({ ...lake, uniqueId: `${idx}-1` });
            }
          });

          // Sort by area (bigger first) but keep duplicates
          const sortedLakes = duplicatedLakes.sort((a: any, b: any) => 
  ...
  // Convert lake coordinates to simplified path - more aggressive simplification
  const getSimplifiedPath = (coordinates: number[][][]) => {
    ...
    // Create more simplified path - take fewer points for better performance
    const step = Math.max(1, Math.floor(ring.length / 8)); // Even fewer points
  ...
  return (
    <>
      {/* Lake outlines in background - optimized for performance */}
          ...
          return (
            <svg
              ...
              style={{
                ...
                width: `${size}px`,
                height: `${size}px`,
                opacity: isDisappearing ? 0 : (index < visibleLakes ? 0.3 : 0),
                transform: `
                  translate(-50%, -50%) 
                ...
                transition: isDisappearing 
                  ...
                  : 'opacity 0.6s ease-out, transform 0.6s ease-out', // Simplified transitions
                transitionDelay: isDisappearing 
                  ? '0ms' 
                  : `${position.appearDelay}ms`,
                willChange: 'opacity, transform' // Hint to browser for GPU acceleration
              }}
              viewBox="0 0 100 100"
            >
              <path
